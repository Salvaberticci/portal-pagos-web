...
<body>
    <div id="page-loading" class="loading-overlay" style="display:flex;">
        ...
        <div class="loading-sub">Consultando datos del servicio</div>
    </div>
    <?php
    if (ob_get_level())
        ob_end_flush();
    ...
    <div id="page-content" style="display:none;">
        ...
        <div class="container main-container animate-fade py-4" style="max-width:520px;">
            ...
            <div id="pago_mensajes" data-error="<?php echo htmlspecialchars($pago_err); ?>"
                data-exito="<?php echo htmlspecialchars($pago_exito); ?>"
                data-detalles='<?php echo $pago_data ? htmlspecialchars(json_encode($pago_data), ENT_QUOTES, 'UTF-8') : ''; ?>'>
            </div>
            ...
            <form id="paymentForm" action="procesar_pago_cliente.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="monto_usd" id="input_monto_usd"
                    value="<?php echo number_format($monto_a_pagar, 2, '.', ''); ?>">
                    ...
                    <div class="recibo-select-body" id="reciboBody">
                        ...
                        <?php else: ?>
                            <?php foreach ($invoices_json as $inv): ?>
                                <div class="recibo-select-item<?php echo $inv['vencida'] ? ' vencida' : ''; ?>"
                                    data-id="<?php echo $inv['id']; ?>">
                                    <input type="radio" class="recibo-select-check" name="recibo"
                                        id="recibo_<?php echo $inv['id']; ?>" data-id="<?php echo $inv['id']; ?>"
                                        onchange="toggleRecibo(this)" <?php echo $inv['preseleccionado'] ? ' checked' : ''; ?>>
                                    <label class="recibo-select-label" for="recibo_<?php echo $inv['id']; ?>">
                                        ...
                                        <div class="recibo-select-content">
                                            <div class="recibo-select-top">
                                                <span class="recibo-select-folio">Recibo
                                                    #<?php echo htmlspecialchars($inv['folio']); ?></span>
                                                <span class="recibo-select-monto-bs">Bs
                                                    <?php echo number_format($inv['monto_bs'], 2, ',', '.'); ?></span>
                                            </div>
                                            <div class="recibo-select-bottom">
                                                <span
                                                    class="recibo-select-desc"><?php echo htmlspecialchars($inv['descripcion']); ?></span>
                                                <?php if ($inv['vencida']): ?>
                                                    <span class="recibo-select-badge badge bg-danger ms-1">Vencida</span>
                                                <?php endif; ?>
                                                <?php if (!empty($inv['fecha_vencimiento'])): ?>
                                                    <span class="recibo-select-fecha ms-1">Vence:
                                                        <?php echo date('d/m/Y', strtotime($inv['fecha_vencimiento'])); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </label>
                ...
                <div class="glass-panel p-4 mb-3">
                    ...
                    <div class="metodo-btns-grid">
                        <?php foreach ($metodosFiltrados as $metodo => $bancosMetodo): ?>
                            <button type="button" class="metodo-btn" data-metodo="<?php echo htmlspecialchars($metodo); ?>"
                                onclick="selectMetodo('<?php echo htmlspecialchars($metodo); ?>')">
                                <?php if ($metodo === 'Zelle'): ?>
                ...
                <div class="glass-panel p-4 mb-3 d-none" id="panel-banco">
                    ...
                    <div class="mt-2 text-end">
                        <button type="button" class="btn btn-sm btn-glass copy-btn" id="btn_copiar"
                            onclick="copiarDatos()">
        ...
        <div id="loadingOverlay"
            style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:99999;justify-content:center;align-items:center;flex-direction:column;">
            <div
                style="width:50px;height:50px;border:4px solid rgba(255,255,255,0.2);border-top-color:#3b82f6;border-radius:50%;animation:loadingSpin 0.8s linear infinite;">
            </div>
        ...
        <script>
            ...
            function selectMetodo(metodo) {
                ...
                // Siempre permitir 6 a 20 dígitos y el backend se encargará de extraer los últimos 6
        ...
        <style>
            ...
            .recibo-select-item:hover {
                background: rgba(255, 255, 255, 0.03);
            }

            .recibo-select-item.vencida {
            ...
            .modal-content.glass-panel .modal-footer {
